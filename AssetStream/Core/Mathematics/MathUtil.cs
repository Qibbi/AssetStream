...
namespace Core.Mathematics
{
    public static class MathUtil
    {
        ...
        {
            return value == 0u ? 32u : 31u - FloorLog2(value);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ulong CountLeadingZeros(ulong value)
        {
            return value == 0UL ? 64UL : 63UL - FloorLog2(value);
        }
        ...
        {
            return grid == 0.0f ? x : FloorToFloat((x + (0.5f * grid)) / grid) * grid;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static double GridSnap(double x, double grid)
        {
            return grid == 0.0 ? x : FloorToDouble((x + (0.5 * grid)) / grid) * grid;
        }
